    ...
    (function(Highcharts) {
        ...
        Highcharts.downloadURL = function(dataURL, filename) {
            ...
            if (nav.msSaveOrOpenBlob) {
                console.log("IE blob specific implementation?");
            ...
            if (a.download !== undefined) {
                console.log("HTML 5 download attr..");
                a.href = dataURL;
        ...
        Highcharts.svgToDataUrl = function(svg) {
            console.log("Saving to Data Url...");
        ...
        Highcharts.downloadSVGLocal = function(svg, options, failCallback, successCallback) {
            console.log("method: download SVG Local");
            var svgurl,
                blob,
            ...
            if (imageType === 'image/svg+xml') {
                ...
                Highcharts.imageToDataUrl(svgurl, imageType, { /* args */ }, scale, function(imageURL) {
                        // Success
                        try {
                            console.log("try to get PNG by rendering on canvas");
                        ...
                        console.log("creating new and untainted canvas..");
                        var canvas = doc.createElement('canvas'),
        ...
        Highcharts.Chart.prototype.getSVGForLocalExport = function(options, chartOptions, failCallback, successCallback) {
            ...
            console.log("end we get the image svg");
            try {
                // If there are no images to embed, the SVG is okay now.
                console.log("SVG is OK");
                if (!images.length) {
                    console.log("Using SVG of chart copy to SanitizeSVG...");
        ...
        Highcharts.Chart.prototype.exportChartLocal = function(exportingOptions, chartOptions) {
            ...
            var chart = this,
                ...
                fallbackToExportServer = function() {
                    console.log("method: fallbackToExportServer");
                    if (options.fallbackToExportServer === false) {
                        console.log("fallbackToExportServer false!");
                        if (options.error) {
                            console.log("Options error!");
                            options.error();
                        } else {
                            console.log("Fallback to export server disabled");
                            throw 'Fallback to export server disabled';
                        }
                    } else {
                        console.log("Export chart using net");
            ...
            if ((isMSBrowser && options.imageType !== 'image/svg+xml' || options.imageType !== 'application/pdf') && chart.container.getElementsByTagName('image').length) {
                console.log("Export to JPEG/PNG..");
                fallbackToExportServer();
                return;
            }
            console.log("Getting SVG for Local Export...");
